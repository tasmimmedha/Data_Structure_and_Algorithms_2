...
MST connects all vertices with minimum total edge weight without cycles; Kruskal sorts edges and adds non-cycle forming ones using Union-Find (O(E log E)), Prim grows from vertex using priority queue (O(E log V)).Both suit dense/sparse graphs differently.[web:9] Pseudocode for Kruskal: Sort edges by weight; initialize Union-Find; for each edge, if not same set, union and add to MST until V-1 edges.
